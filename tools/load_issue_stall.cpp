...
struct LoopOutputConfig {
  ...
  static LoopOutputConfig from_flags() {
    ...
    if (cfg.aggregate_wave_only) cfg.aggregate_wave = true;
...
std::optional<uint64_t> parse_dispatch_id_from_path(const std::string& path) {
  ...
  if (digit_pos >= name.size() ||
      !std::isdigit(static_cast<unsigned char>(name[digit_pos])))
    return std::nullopt;
...
void analyze_mfma_coexec(trace_decoder_context& ctx,
  ...
  for (size_t i = 0; i < wave.instructions_size; i++) {
    ...
    if (my_rocperf_tool::is_null_pc(inst)) continue;
    ...
    if (cache_it->second) {
      ...
      if (last_mfma_coexec_end > 0 && issue_point < last_mfma_coexec_end) {
        fmt::print(stderr,
                   ...
                   last_mfma_coexec_end, last_mfma_coexec_end - issue_point);
      }
...
void compute_relative_issue(trace_decoder_context& ctx,
                            const rocprofiler_thread_trace_decoder_wave_t& wave,
                            const my_rocperf_tool::BackEdge& edge) {
...
std::vector<rocprofiler_thread_trace_decoder_inst_t> coalesce_barrier_tokens(
  ...
  for (size_t i = 0; i < wave.instructions_size;) {
    ...
    merged.stall = total_stall > kStallMax ? kStallMax
    ...
    case ROCPROFILER_THREAD_TRACE_DECODER_RECORD_WAVE:
      ...
      for (size_t wave_n = 0; wave_n < trace_size; wave_n++) {
        ...
        for (size_t j = 0; j < wave.instructions_size; j++) {
          ...
          if (ctx.verify_duration &&
              inst.duration < static_cast<int32_t>(inst.stall)) {
            fmt::print(stderr,
...
void dump_states(my_rocperf_tool::Disassembler& disas,
                 my_rocperf_tool::InstStatistics& stats, bool decode_all) {
...
std::vector<LoopInstInfo> collect_loop_insts(
    my_rocperf_tool::Disassembler& disas,
    my_rocperf_tool::InstStatistics& stats,
    const my_rocperf_tool::BackEdge& edge, const PerLoopMap& all_bubbles,
  ...
  const auto& rel_issues =
      ri != all_rel_issues.end() ? ri->second : empty_pc_map;
  ...
  while (sec_offset <= end_offset && !bytes.empty()) {
    ...
    LoopInstInfo info{addr, std::move(inst_str), 0, 0, 0, 0, 0, 0,
                      0,    !stats_range.empty()};
    if (info.has_stats) {
      ...
      if (bi_it != bubbles.end()) info.bubble = bi_it->second;
      auto ri_it = rel_issues.find(pc);
      if (ri_it != rel_issues.end()) info.rel_issue = ri_it->second;
    }
...
const char* bubble_color(const LoopInstInfo& ii, uint64_t max_bubble) {
  ...
  if (ratio <= 0.25) return "\033[33m";  // yellow: low
  if (ratio <= 0.75) return "\033[31m";  // red: medium
...
LoopSummary compute_loop_summary(const my_rocperf_tool::LoopStats& s,
  ...
  for (const auto& ii : insts) {
    ...
    if (ii.has_stats && ii.n > total_n) total_n = ii.n;
    if (llvm::StringRef(ii.inst_str).starts_with("v_mfma")) sum.mfma_count++;
  }
  ...
  if (total_n > 0) sum.avg_bubble = static_cast<double>(total_bubble) / total_n;
            ...
            [](const LoopRecord& a, const LoopRecord& b) {
              return std::tie(a.back_edge.code_object_id,
                     ...
                     std::tie(b.back_edge.code_object_id,
                              b.back_edge.target_addr, b.back_edge.source_addr);
            });
  return result;
}
...
std::vector<LoopGroup> build_loop_groups(
    const std::vector<my_rocperf_tool::WaveLoopInfo>& wave_loops,
...
void dump_loop_json(TraceBuf& out, const my_rocperf_tool::BackEdge& e,
  ...
  if (wave_count.has_value()) writef(out, ",\"wave_count\":{}", *wave_count);
  writef(out,
    ...
    for (const auto& ii : insts) {
      ...
      if (cfg.show_stats && ii.has_stats) {
        ...
        if (cfg.show_avg) {
          ...
          if (cfg.show_mfma_coexec) writef(out, ",\"bubble\":{}", ii.bubble);
...
void dump_csv_header(TraceBuf& out, const LoopOutputConfig& cfg) {
  if (cfg.aggregate_wave) writef(out, "record_type,");
  writef(out, "dispatch_id,cu,simd,wave_id,");
  if (cfg.aggregate_wave) writef(out, "wave_count,");
  writef(out,
         "target_addr,source_addr,code_object,"
         "iterations,total_duration,total_stall,total_idle,"
         "loop_avg_dur,loop_avg_stall,loop_avg_idle,");
  if (cfg.show_mfma_coexec) writef(out, "loop_avg_bubble,");
  ...
  if (cfg.show_contents) {
    ...
    if (cfg.show_avg) {
      ...
      if (cfg.show_mfma_coexec) writef(out, ",avg_bubble");
      writef(out, ",avg_cycle");
    } else {
      writef(out, "dur,stall,idle,issue");
      if (cfg.show_mfma_coexec) writef(out, ",bubble");
...
void dump_csv_loop_rows(TraceBuf& out, const LoopOutputConfig& cfg,
                        uint64_t dispatch_id, const LoopGroup& g,
                        const LoopRecord& lr,
                        const std::vector<LoopInstInfo>& insts) {
  ...
  auto print_loop_fields = [&]() {
    ...
    if (cfg.show_mfma_coexec) writef(out, "{:.2f},", sum.avg_bubble);
  ...
  for (const auto& ii : insts) {
    ...
    if (cfg.show_avg) {
      if (ii.n > 0) {
        ...
        writef(out, ",,,,");
        if (cfg.show_mfma_coexec) writef(out, ",");
        writef(out, ",");
      }
      writef(out, "\n");
    } else {
      writef(out, "{},{},{},{}", ii.dur, ii.stall, ii.idle, ii.issue);
      if (cfg.show_mfma_coexec) writef(out, ",{}", ii.bubble);
      writef(out, ",{}\n", ii.rel_issue);
    }
  }
}
...
// trailing comma. Caller is responsible for the surrounding "[\n" / "\n]\n"
// and for inserting ",\n" between non-empty per-trace bodies.
void dump_loops_json(const DumpCtx& ctx, const std::vector<LoopGroup>& groups) {
  bool first_entry = true;
  for (const auto& g : groups) {
    if (!first_entry) writef(ctx.out, ",\n");
    first_entry = false;
    if (g.is_aggregate)
      writef(ctx.out, "  {{\"dispatch_id\":{},\"aggregate\":true,\"loops\":[\n",
             ctx.dispatch_id);
    else
      writef(ctx.out,
    ...
    for (const auto& lr : g.loops) {
      ...
      first_loop = false;
      const auto& insts =
          get_loop_insts(ctx.cache, ctx.disas, ctx.stats, lr.back_edge,
                         ctx.bubbles, ctx.rel_issues);
      auto wave_count = g.is_aggregate ? std::optional<uint32_t>(lr.wave_count)
                                       : std::nullopt;
...
void dump_loops_csv(const DumpCtx& ctx, const std::vector<LoopGroup>& groups) {
  for (const auto& g : groups) {
    for (const auto& lr : g.loops) {
      const auto& insts =
          get_loop_insts(ctx.cache, ctx.disas, ctx.stats, lr.back_edge,
...
void dump_loop_text_stats(TraceBuf& out, const my_rocperf_tool::LoopStats& s,
                          const std::vector<LoopInstInfo>& insts,
                          const LoopOutputConfig& cfg) {
  auto sum = compute_loop_summary(s, insts);
  writef(out, "    avg/iter: dur={:.2f} stall={:.2f} idle={:.2f}", sum.avg_dur,
         sum.avg_stall, sum.avg_idle);
  if (cfg.show_mfma_coexec) writef(out, " bubble={:.2f}", sum.avg_bubble);
  writef(out, " mfma_time={} ({} mfma * 16) mfma_util={:.1f}%\n", sum.mfma_time,
         sum.mfma_count, sum.mfma_util * 100.0);
  if (!cfg.show_contents) return;
  ...
  if (cfg.show_addr) writef(out, "{:<14} ", "addr");
  writef(out, "instruction\n");

  uint64_t max_bubble = 0;
  if (cfg.color_mfma_bubble) {
    for (const auto& ii : insts)
      if (ii.bubble > max_bubble) max_bubble = ii.bubble;
  }
  for (const auto& ii : insts) {
    ...
    if (cfg.show_stats && ii.has_stats) {
      ...
      if (cfg.show_mfma_coexec) writef(out, "{:<10} ", "");
      writef(out, "{:<10} ", "");
    }
    if (cfg.show_addr) writef(out, "0x{:x}:  ", ii.addr);
    writef(out, "{}", ii.inst_str);
    if (cfg.color_mfma_bubble && ii.has_stats) writef(out, "\033[0m");
    writef(out, "\n");
  }
}

void dump_loops_text(const DumpCtx& ctx, const std::vector<LoopGroup>& groups) {
  for (const auto& g : groups) {
    if (g.is_aggregate)
...
int run_trace(TraceBuf& out, TraceSummary& summary,
  ...
  trace_decoder_context decoder_ctx(disas);
  {
    ...
    decoder_ctx.dispatch_id =
        dispatch_id_override.value_or(att_path.dispatch_id);
  ...
  uint64_t loops = 0;
  for (const auto& wl : decoder_ctx.wave_loops) loops += wl.loops.size();
  summary.loops = loops;

  {
    ...
    if (decoder_ctx.detect_loops_flag) {
      LoopInstCache cache;
      DumpCtx dump_ctx{
          out,   decoder_ctx.disas,         decoder_ctx.stats,
          cfg,   decoder_ctx.bubble_totals, decoder_ctx.rel_issue_totals,
          cache, decoder_ctx.dispatch_id};
...
int run_main(const std::string& att_output_dir_path) {
  ...
  if (cfg.format == LoopOutputFormat::Json) std::fputs("\n]\n", stdout);
  ...
  // a trace here is one .att file = one shader engine of one dispatch.
  {
    ...
    if (detect) {
      ...
      for (const auto& [id, d] : by_dispatch)
        if (d.waves > 0 && d.loops == 0) loop_free.push_back(id);
      if (!loop_free.empty()) {
        fmt::print(stderr,
        ...
        if (loop_free.size() < by_dispatch.size()) {
          ...
          for (uint64_t id : loop_free) writef(ids, " {}", id);
          writef(ids, "\n");
